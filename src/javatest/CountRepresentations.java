...
public class CountRepresentations {
	...
	static int countRepresentations(int amount) {
		int count = 0;
		int remainder = 0;
		
		for (int rupee20=0; rupee20<= amount; rupee20++) {
			for (int rupee10=0; rupee10<= 20; rupee10++) {
				for (int rupee5=0; rupee5<= 10; rupee5++) {
					for (int rupee2=0; rupee2<= 5; rupee2++) {
						for (int rupee1=0;  rupee1<= 2; rupee1++) 
						{ 
							if (amount != 0) 
								count++; 
		...
		System.out.println("johnny: "+count);
		return count;
	}
